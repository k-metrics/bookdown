...
　\textbf{R}はデータ分析に必要となるデータのハンドリングや可視化、モデリング、そして、レポートといった様々な機能をほとんど無料で利用することができます。\href{https://cran.r-project.org/}{CRAN(The Comprehensive R Archive Network)}と呼ばれる\textbf{R}のリポジトリには15,000を超えるパッケージ（ライブラリ）が登録されています。それらのパッケージが網羅する分野は\href{https://cran.r-project.org/web/views/}{CRAN Task Views}で整理されています。対象分野は古典的な統計や金融統計から最新の機械学習・ベイズ統計など40を超えています。その中でも特筆すべき分野は\href{https://cran.r-project.org/web/views/ReproducibleResearch.html}{Reproducible Research}と呼ばれる再現可能性の分野です。再現可能性とは聞き慣れない言葉だと思いますが、\textbf{R}の世界では「ある分析結果を再分析した際に同じ結果が得られること」を意味しています。元々はResearchとあるように科学的研究の分野で使われている言葉です。 他のプログラミング言語で、Reproducible Researchをサポートしているという話は聞いたことがありませんので、このあたりは\textbf{R}の大きな特徴といえます。

　また、\textbf{R}は逐次実行するインタプリタ型言語ですのでソフトウェアメトリクス分析のような探索的分析（Exploratory data analysis）に適しています。加えて非常にフレンドリーかつ活発なコミュニティーが日本でも形成されていますので、悩んだ時などに気軽に質問・相談ができるのも大きな強みです。ちなみに本書は\textbf{R}\href{https://bookdown.org/}{\textbf{\texttt{bookdown}}}パッケージという文書作成に特化したパッケージを利用して作成してます。
...
　\textbf{R}で、どのようなことができるかを簡単な分析を通して見てみます。対象データは\href{https://en.wikipedia.org/wiki/Iris_flower_data_set}{フィッシャーのあやめ（Fisher's or Anderson's iris）}\index{iris dataset}呼ばれる150行の下表のようなデータです。
...
　まずは、データがどのような特徴を持っているか要約統計量（記述統計量）で確認します。
...
　萼片（Sepal）の長さと幅、花弁（Petal）の長さと幅は、量的変数でその最小値（Min.）、第1四分位値（1st Qu.）、中央値（Median）、平均値（Mean）、第3四分位値（3rd Qu.）、最大値（Max.）が表示されているのが分かります。この量的変数から萼片（Sepal）より花弁（Petal）の方が小さい傾向にあると推測できます。一方、品種（Species）は質的変数（カテゴリ変数）なので、その個数が表示されます。三品種あり、どの品種も同じ数だけデータがあることが分かります。

　量的変数の傾向の違いを箱ひげ図で可視化して確認してみます。

\begin{figure}[H]

{\centering \includegraphics[width=0.9\linewidth,]{R_files/figure-latex/unnamed-chunk-4-1} 
...
　数値で比較するよりグラフ化（可視化）した方が要約統計量の傾向を掴みやすいことが分かります。次に花弁（Petal）と萼片（Sepal）の幅と長さの関係を散布図で可視化して確認します。

\begin{figure}[H]

{\centering \subfloat[花弁\label{fig:unnamed-chunk-5-1}]{\includegraphics[width=0.5\linewidth,]{R_files/figure-latex/unnamed-chunk-5-1} }\subfloat[萼片\label{fig:unnamed-chunk-5-2}]{\includegraphics[width=0.5\linewidth,]{R_files/figure-latex/unnamed-chunk-5-2} }
...
　花弁（Petal）の幅と長さに関係があり、萼片（Sepal）の方には関係がないように見えますので、個々の幅と長さの相関関係を見ておきます。
...
　花弁（Petal）の方には強い相関があり、萼片（Sepal）の方には相関がないことが分かります。なお、この出力の結果の読み方は別途、説明します。では、花弁（Petal）の幅と長さの具体的な関係を回帰モデル（回帰式）を用いて表してみましょう。
...
{\centering \subfloat[花弁の回帰関係\label{fig:unnamed-chunk-8-1}]{\includegraphics[width=0.5\linewidth,]{R_files/figure-latex/unnamed-chunk-8-1} }\subfloat[萼片の回帰関係\label{fig:unnamed-chunk-8-2}]{\includegraphics[width=0.5\linewidth,]{R_files/figure-latex/unnamed-chunk-8-2} }
...
　探索的データ分析では、このように対象のデータの確認、データの変形と可視化、モデルの作成を繰り返すことで最適な（予測）モデルを探ります。このような流れが基本となる分析プロセスで、下図のような関係があります。
...
　実際には分析対象のデータが\textbf{R}に組み込まれている訳ではありませんので、対象となるデータを読み込み（インポート）、処理がしやすいように整形してから可視化などを行います。最後に分析結果を報告（レポート）しますので、実際の分析プロセスは下図のようになります。
...
　分析プロセスを実際に回すには\textbf{R}などのプログラム（プログラミング）で円滑に回せるよう仕組みが必要となります。それを加えたものが「Data Science Workflow」と呼ばれるフロー図です。
...
　Hadlyの図には前節の図にはない\textbf{Infer}と\textbf{Automate}というプロセスが入っていますが、本書では\textbf{Infer}は\textbf{Model}に、\textbf{Automate}は\textbf{Communicate}に含まれるものとして分類しています。
...
　データ分析のすべてのプロセス（\textbf{Import}〜\textbf{Communicate}）で必要となるツールが\textbf{Program}です。\textbf{R}では様々なパッケージを使うことで外部アプリケーションとの連動を図り、\textbf{R}だけで全てのプロセスが完結するR Ecosystemと呼べる体系が出来ています。

\hypertarget{import}{%
\subsection*{Import}\label{import}}
\addcontentsline{toc}{subsection}{Import}

　分析対象となるデータを分析環境に取り込むのが\textbf{Import}プロセスです。データは様々な形式（文字コード、ファイル形式など）で保存されていますので、それらに見合った方法でインポートする必要があります。

\hypertarget{tidy}{%
\subsection*{Tidy}\label{tidy}}
\addcontentsline{toc}{subsection}{Tidy}

　インポートしたデータは必ずしもデータ分析に適した形式になっているとは限りませんので、\textbf{R}で扱いやすいような形式（Tidy Data）にします。\href{https://www.jstatsoft.org/article/view/v059i10}{Tidy Data}\citep{R-TidyData}\index{Tidy Data}はデータ分析において非常に重要な概念で、以下の条件を満たしたデータを意味します。
...
\caption{正規分布に近い分布}\label{fig:unnamed-chunk-33}
\end{figure}
...
{\centering \includegraphics[width=0.9\linewidth,]{R_files/figure-latex/unnamed-chunk-35-1} 

}

\caption{右に歪んだ分布（左に偏った分布）}\label{fig:unnamed-chunk-35}
\end{figure}
...
{\centering \includegraphics[width=0.9\linewidth,]{R_files/figure-latex/unnamed-chunk-37-1} 

}

\caption{左に歪んだ分布（右に偏った分布）}\label{fig:unnamed-chunk-37}
\end{figure}
...
{\centering \includegraphics[width=0.9\linewidth,]{R_files/figure-latex/unnamed-chunk-39-1} 

}

\caption{正規分布に近い分布}\label{fig:unnamed-chunk-39}
\end{figure}
...
{\centering \includegraphics[width=0.9\linewidth,]{R_files/figure-latex/unnamed-chunk-41-1} 

}

\caption{中心部が高く幅の狭い尖った分布}\label{fig:unnamed-chunk-41}
\end{figure}
...
{\centering \includegraphics[width=0.9\linewidth,]{R_files/figure-latex/unnamed-chunk-43-1} 

}

\caption{中心部が低く横に広がった分布}\label{fig:unnamed-chunk-43}
\end{figure}
...
\caption{Google Colab, Theme: dark}\label{fig:unnamed-chunk-125}
\end{figure}
...
\caption{Upload notebook file}\label{fig:unnamed-chunk-126}
\end{figure}
...
\caption{Upload from GitHub}\label{fig:unnamed-chunk-127}
\end{figure}
...
\caption{Warning dialog}\label{fig:unnamed-chunk-128}
\end{figure}
...
\caption{RStudio Cloud, beta}\label{fig:unnamed-chunk-129}
\end{figure}
...
\caption{Initial View}\label{fig:unnamed-chunk-130}
\end{figure}
...
\caption{Packages Manager}\label{fig:unnamed-chunk-131}
\end{figure}
...
\caption{Install Dialog}\label{fig:unnamed-chunk-132}
\end{figure}
...
\caption{RStudio Desktop, Windows}\label{fig:unnamed-chunk-140}
\end{figure}
...
\caption{RStudio Pane Layout, Windows}\label{fig:unnamed-chunk-141}
\end{figure}
...
\caption{R Notebook file}\label{fig:unnamed-chunk-142}
\end{figure}
...
\caption{R Notebook saved file}\label{fig:unnamed-chunk-143}
\end{figure}
...
\caption{R Notebook insert chunk}\label{fig:unnamed-chunk-144}
\end{figure}
...
\caption{R Notebook first code}\label{fig:unnamed-chunk-145}
\end{figure}
...
\caption{R Notebook run code}\label{fig:unnamed-chunk-146}
\end{figure}
...
\caption{R Notebook option}\label{fig:unnamed-chunk-147}
\end{figure}
...
\caption{RStudio Cloud, beta}\label{fig:unnamed-chunk-148}
\end{figure}
...
\caption{Exploratory Public}\label{fig:unnamed-chunk-149}
\end{figure}
