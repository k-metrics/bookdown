...
参考文献・資料\footnote{PDF形式のみ表示、()内は文書内ハイパーリンク} & (著者・作者, 作成年) & (Wickham, 2021) \\
\bottomrule
\end{longtable}
...
インポートしたデータは必ずしもデータ分析に適した形式になっているとは限りませんので、\textbf{R}で扱いやすいような形式（Tidy Data）にします。\href{https://www.jstatsoft.org/article/view/v059i10}{Tidy Data}(\protect\hyperlink{ref-R-TidyData}{Wickham 2014})はデータ分析において非成に重要な概念で、以下の条件を満たしたデータを意味します。
...
ここでは、主要なパッケージをいくつか紹介するに留めておきます。詳細は各分析プロセスの章、または、『RStartHere』(\protect\hyperlink{ref-RStartHere:GitHub}{RStudio, PBC 2021})をご覧ください。
...
\item
  ...
  \tightlist
  \item
    様々なテキスト形式テーブルを読み込むためのパッケージ(\protect\hyperlink{ref-R-readr}{Wickham and Hester 2020})
  \end{itemize}
\item
  \textbf{\texttt{readxl}}
  ...
  \tightlist
  \item
    Microsoft Excelのファイルを読み込むためのパッケージ(\protect\hyperlink{ref-R-readxl}{Wickham and Bryan 2019})
  \end{itemize}
\item
  \textbf{\texttt{rvest}}
  ...
  \tightlist
  \item
    Webスクレイピングのためのパッケージ(\protect\hyperlink{ref-R-rvest}{Wickham 2021b})
  \end{itemize}
\item
  ...
  \tightlist
  \item
    Google API v4 経由でスプレッドシートを読み込むためのパッケージ(\protect\hyperlink{ref-R-googlesheets4}{Bryan 2021})
  \end{itemize}
\item
  \textbf{\texttt{DBI}}
  ...
  \tightlist
  \item
    SQL系の各種データベースと接続するためのパッケージ(\protect\hyperlink{ref-R-DBI}{R Special Interest Group on Databases (R-SIG-DB), Wickham, and Müller 2021})
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    Tidy Dataの作成を強力にサポートしてくれるパッケージ(\protect\hyperlink{ref-R-tidyr}{Wickham 2021c})
  \end{itemize}
\item
  \textbf{\texttt{tibble}}
  ...
  \tightlist
  \item
    より厳密にTidy Dataを扱うためのパッケージ(\protect\hyperlink{ref-R-tibble}{Müller and Wickham 2021})
  \end{itemize}
\item
  \textbf{\texttt{zoo}}
  ...
  \tightlist
  \item
    時系列（TS）データを効率よく扱うためのパッケージ(\protect\hyperlink{ref-R-zoo}{Zeileis, Grothendieck, and Ryan 2021})
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    統一された文法で描画が行えるパッケージ(\protect\hyperlink{ref-R-ggplot2}{Wickham et al. 2020})
  \end{itemize}
\item
  ...
  \tightlist
  \item
    JavaScriptのウィジェットを利用できるパッケージ(\protect\hyperlink{ref-R-htmlwidgets}{Vaidyanathan et al. 2020})
  \end{itemize}
\item
  ...
  \tightlist
  \item
    \textbf{\texttt{ggplot2}}オブジェクトをひとつにまとめてレイアウトするためのパッケージ(\protect\hyperlink{ref-R-patchwork}{Pedersen 2020})
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    Tidy Dataを様々な方法で操作するためのパッケージ(\protect\hyperlink{ref-R-dplyr}{Wickham et al. 2021})
  \end{itemize}
\item
  \textbf{\texttt{stringr}}
  ...
  \tightlist
  \item
    文字列処理をするためのパッケージ(\protect\hyperlink{ref-R-stringr}{Wickham 2019})
  \end{itemize}
\item
  \textbf{\texttt{forcats}}
  ...
  \tightlist
  \item
    因子型を操作するためのパッケージ(\protect\hyperlink{ref-R-forcats}{Wickham 2021a})
  \end{itemize}
\item
  ...
  \tightlist
  \item
    日付データを簡単に変換するためのパッケージ(\protect\hyperlink{ref-R-lubridate}{Spinu, Grolemund, and Wickham 2021})
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    各種モデリング結果を整然データにするためのパッケージ(\protect\hyperlink{ref-R-broom}{Robinson, Hayes, and Couch 2021})
  \end{itemize}
\item
  ...
  \tightlist
  \item
    機械学習を中心としたモデリングフレームワーク(\protect\hyperlink{ref-tidymodels2020}{Kuhn and Wickham 2020}, \protect\hyperlink{ref-R-tidymodels}{2021})
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    \textbf{R}のコードと実行結果をPDFやHTMLなどに埋め込むためのパッケージ(\protect\hyperlink{ref-knitr2014}{Xie 2014}, \protect\hyperlink{ref-R-knitr}{2021b})
  \end{itemize}
\item
  ...
  \tightlist
  \item
    マークダウン書式を用いてレポートを作成するためのパッケージ(\protect\hyperlink{ref-R-rmarkdown}{Allaire et al. 2021})
  \end{itemize}
\item
  ...
  \tightlist
  \item
    書籍や長いドキュメンを作成するためのパッケージ(\protect\hyperlink{ref-bookdown2016}{Xie 2016}, \protect\hyperlink{ref-R-bookdown}{2021a})
  \end{itemize}
\item
  \textbf{\texttt{shiny}}
  ...
  \tightlist
  \item
    インタラクティブなWebアプリケーションを作成するためのパッケージ(\protect\hyperlink{ref-R-shiny}{Chang et al. 2021})
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    パイプ演算子（\texttt{\%\textgreater{}\%}）によるスムースな処理を実現するパッケージ(\protect\hyperlink{ref-R-magrittr}{Bache and Wickham 2020})
  \end{itemize}
\item
  \textbf{\texttt{purrr}}
  ...
  \tightlist
  \item
    関数型プログラミングによる反復処理を提供するパッケージ(\protect\hyperlink{ref-R-purrr}{Henry and Wickham 2020})
  \end{itemize}
\end{itemize}
...
\textbf{R}の文法説明が延々と続いてもつまらないので、本書では少し実用的な面から\textbf{R}のプログラミングを説明します。本チャプターでは『統計のはなし【改訂版】』(\protect\hyperlink{ref-ToukeinoHanashi}{大村 2020})や『統計解析のはなし【改訂版】』(\protect\hyperlink{ref-ToukeiKaisekinoHanashi}{大平 2006})を参考書として説明します。個々の詳細については、これらの書籍などを参照してください。 なお、基本文法については\protect\hyperlink{Appendix-RBasics}{Appendix R Basics}をご覧ください。
...
しかし、\textbf{R}にはこの標本分散（\(s^2\)）を求める関数は用意されていませんが、不偏分散（\(\hat{\sigma}^2\)）を求める\texttt{var()}関数が用意されています。
...
標本分散（\(s^2\)）の定義式\eqref{eq:variance}と不偏分散（\(\hat{\sigma}^2\)）の定義式\eqref{eq:var}から標本分散（\(s^2\)）と不偏分散（\(\hat{\sigma}^2\)）の間には関係があることが分かります。
...
標本分散（\(s^2\)）と不偏分散（\(\hat{\sigma}^2\)）の関係式\eqref{eq:variance-var}を用いて\textbf{R}で以下の二つのデータの標本分散を求めなさい。
...
\hypertarget{ux9591ux8a71-1}{%
\subsubsection*{閑話}\label{ux9591ux8a71-1}}
\addcontentsline{toc}{subsubsection}{閑話}

平均（分布の重心）と分散（分布の広がり）は、歪度（分布の左右非対称度合い）、尖度（分布の峰の尖り度合い）を含めて「モーメントから求められる要約統計量」と言われることがあります。 n個のデータの平均値を

\[\bar{x} = \frac{1}{n}\sum_{i = 1}^{n}x_i\] \begin{equation}
\end{equation}

とした場合、平均値まわりの\(m\)次の中央モーメント\(\bar{x}_m\)を

\[\bar{x}_m = \frac{1}{n}\sum_{i = 1}^{n}(x_i - \bar{x})^m　　(m = 2, 3, 4)\] \begin{equation}
\end{equation}

と定義します。\(m = 2\)の場合、

\[\bar{x}_2 = \frac{1}{n}\sum_{i = 1}^{n}(x_i - \bar{x})^2 = s^2\] \begin{equation}
\end{equation}

となりますので、\(\bar{x}_2\)は分散\(s^2\)であることがわかります。\(m = 3\)の場合、

\[\bar{x}_3 = \frac{1}{n}\sum_{i = 1}^{n}(x_i - \bar{x})^3\] \begin{equation}
\end{equation}

となり、これを標準偏差の三乗（\(s^3\)）で割ったもの

\[\gamma_1 = \frac{\bar{x}_3}{s^3}\] \begin{equation}
\end{equation}

を歪度（\(\gamma_1\)）と呼びます。\(m = 4\)の場合、

\[\bar{x}_4 = \frac{1}{n}\sum_{i = 1}^{n}(x_i - \bar{x})^4\] \begin{equation}
\end{equation}
...
\hypertarget{ux30b3ux30e9ux30e0ux82f1ux8a9e-or-ux30aeux30eaux30b7ux30e3ux8a9e}{%
\subsubsection*{コラム）英語 or ギリシャ語}\label{ux30b3ux30e9ux30e0ux82f1ux8a9e-or-ux30aeux30eaux30b7ux30e3ux8a9e}}
\addcontentsline{toc}{subsubsection}{コラム）英語 or ギリシャ語}

統計では様々な統計量を英語で表記する場合とギリシャ語で表記する場合があり混乱しやすいのですが、統計量を英語で表記されている場合は（標本）統計量、ギリシャ語で表記されている場合は母数の推定統計量（推定量）を意味しています。推定量を表記する場合、推定量であることを意味するハット（\(\hat{}\)）という記号をつけて表記します（例えば、平均値は\(\hat{\mu}\)、分散は\(\hat{\sigma}\)と表記）。ハット（\(\hat{}\)）を省略している書籍や資料も散見されますので混乱しないように注意してください。詳しくは『統計的方法のしくみ』(\protect\hyperlink{ref-ToukeitekiHouhounoSikumi:jbook}{永田 1996})の「3. 母数と統計量の区別」を参照してください。
...
\leavevmode\hypertarget{ref-R-googlesheets4}{}%
Bryan, Jennifer. 2021. \emph{Googlesheets4: Access Google Sheets Using the Sheets API V4}. \url{https://github.com/tidyverse/googlesheets4}.

\leavevmode\hypertarget{ref-R-shiny}{}%
Chang, Winston, Joe Cheng, JJ Allaire, Carson Sievert, Barret Schloerke, Yihui Xie, Jeff Allen, Jonathan McPherson, Alan Dipert, and Barbara Borges. 2021. \emph{Shiny: Web Application Framework for r}. \url{https://shiny.rstudio.com/}.
...
---------. 2021. \emph{Tidymodels: Easily Install and Load the Tidymodels Packages}. \url{https://CRAN.R-project.org/package=tidymodels}.
...
\leavevmode\hypertarget{ref-R-DBI}{}%
R Special Interest Group on Databases (R-SIG-DB), Hadley Wickham, and Kirill Müller. 2021. \emph{DBI: R Database Interface}. \url{https://CRAN.R-project.org/package=DBI}.
...
\leavevmode\hypertarget{ref-RStartHere:GitHub}{}%
RStudio, PBC. 2021. \emph{RStartHere}. \url{https://github.com/rstudio/RStartHere}.
...
Vaidyanathan, Ramnath, Yihui Xie, JJ Allaire, Joe Cheng, Carson Sievert, and Kenton Russell. 2020. \emph{Htmlwidgets: HTML Widgets for r}. \url{https://github.com/ramnathv/htmlwidgets}.

\leavevmode\hypertarget{ref-R-TidyData}{}%
Wickham, Hadley. 2014. {``Tidy Data.''} \emph{Journal of Statistical Software} 59. \url{https://www.jstatsoft.org/article/view/v059i10}.
...
---------. 2021b. \emph{Rvest: Easily Harvest (Scrape) Web Pages}. \url{https://CRAN.R-project.org/package=rvest}.

\leavevmode\hypertarget{ref-R-tidyr}{}%
---------. 2021c. \emph{Tidyr: Tidy Messy Data}. \url{https://CRAN.R-project.org/package=tidyr}.
...
Zeileis, Achim, Gabor Grothendieck, and Jeffrey A. Ryan. 2021. \emph{Zoo: S3 Infrastructure for Regular and Irregular Time Series (z's Ordered Observations)}. \url{https://zoo.R-Forge.R-project.org/}.

\leavevmode\hypertarget{ref-ToukeiKaisekinoHanashi}{}%
大平. 2006. \emph{『統計解析のはなし』}. 改訂版. 日科技連出版. \url{https://www.juse-p.co.jp/products/view/196}.

\leavevmode\hypertarget{ref-ToukeinoHanashi}{}%
大村. 2020. \emph{『統計のはなし』}. 改訂版. 日科技連出版. \url{https://www.juse-p.co.jp/products/view/191}.

\leavevmode\hypertarget{ref-ToukeitekiHouhounoSikumi:jbook}{}%
永田. 1996. \emph{『統計的方法のしくみ』}. First. 日科技連出版. \url{https://www.juse-p.co.jp/products/view/27}.
