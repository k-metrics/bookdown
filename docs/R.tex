...
\textbf{パッケージ四方山話} ＊ 幾何平均を求めるために\texttt{psych}パッケージを利用しましたが、\textbf{R}には総積（\(x_1 \times x_2 \times \cdots \times x_n\)）を求める\texttt{prod()}関数と累乗根（\(x^{\frac{1}{n}}\)）を求めるべき乗演算子（\texttt{\^{}}）がありますので、これを組み合わせれば幾何平均を求めることが可能です。
...
\addcontentsline{toc}{subsection}{モーメント}

本節は\textbf{R}のコードに関わる説明はありませんので、読み飛ばしても構いません。
...
\begin{rmdnote}
\textbf{【コラム】英字？ギリシャ文字？}\\
　統計の書籍では様々な統計量を英字で表記する場合とギリシャ文字で表記する場合があり混乱しやすいのですが、英字（アルファベット）で表記されている場合は標本統計量、ギリシャ文字で表記されている場合は母集団の統計量もしくは母集団の推定統計量を意味しています。 　推定統計量を表記する場合、推定であることを意味するハット（ \(\hat{}\) ）という記号をつけます。 　平均と分散・標準偏差を使って整理すると下表のようになります。
...
平均値や分散・標準偏差はデータの分布が平均値を中心に左右対象になっている場合は有用な統計量ですが、データの分布が大きく歪んでいる場合は、あまり有用とは言えない場合があります。このような場合、平均値に代わり中央値や最頻値、分散・標準偏差に代わり範囲・分位範囲（分位値）を用いるます。これらの統計量はデータの中の位置を用いますので、データの分布に影響されることが小さいロバストネス（頑健性の高い）な統計量です。

中央値は文字通り中央（真ん中）に位置する値で、\texttt{median()}関数を使うことで求めることができます。データが順番に並んでいなくても\texttt{median()}関数でソート処理を行います。
...
\hypertarget{ux7bc4ux56f2ux30ecux30f3ux30b8}{%
\subsection{範囲（レンジ）}\label{ux7bc4ux56f2ux30ecux30f3ux30b8}}
...
以下の\texttt{x}と\texttt{y}は、平均値、中央値、範囲は全て同じ値になるデータです。
...
このようなデータの特徴を見るには最頻値という統計量があります。最頻値とはデータの中で最も頻繁に出てくる値のことで、モードとも呼ばれます。最頻値を求める関数は標準では用意されていませんので\texttt{modeest}パッケージを用います。
...
トリム平均 & & \texttt{mean(x,\ trim\ =\ k),\ k\ =\ 0\ to\ 0.5} \\
幾何平均 & & \texttt{psych::geometric.mean(x)} \\
標本分散 & \(s^2\) & \texttt{(length(x)\ -\ 1)\ /\ length(x)\ *\ var(x)} \\
不偏分散 & \(\hat{\sigma}^2\) & \texttt{var(x)} \\
標本標準偏差 & \(s\) & \texttt{sqrt((length(x)\ -\ 1)\ /\ length(x))\ *\ sd(x)} \\
不偏標本偏差 & \(\hat{\sigma}\) & \texttt{sd(x)} \\
中央値 & & \texttt{median(x)} \\
範囲（レンジ） & \(R\)\footnote{重相関計数を意味する場合もあり} & \texttt{range(x)} or \texttt{diff(range(x))} \\
...
\subsection*{演習 1}\label{ux6f14ux7fd2-1}}
\addcontentsline{toc}{subsection}{演習 1}
...
\subsection*{演習 2}\label{ux6f14ux7fd2-2}}
\addcontentsline{toc}{subsection}{演習 2}
...
\subsection*{演習 3}\label{ux6f14ux7fd2-3-1}}
\addcontentsline{toc}{subsection}{演習 3}

\hypertarget{ux6f14ux7fd2-4}{%
\subsection*{演習 4}\label{ux6f14ux7fd2-4}}
\addcontentsline{toc}{subsection}{演習 4}
...
文中で参照している文献・資料に関しては文献一覧のページを参照してください。なお、文献一覧はPDF版では巻末に「Bibliography」として、HTML版では参照ページの下部の「References」項にあります。
...
\caption{Google Colab, Theme: dark}\label{fig:unnamed-chunk-80}
\end{figure}
...
\caption{Upload notebook file}\label{fig:unnamed-chunk-81}
\end{figure}
...
\caption{Upload from GitHub}\label{fig:unnamed-chunk-82}
\end{figure}
...
\caption{Warning dialog}\label{fig:unnamed-chunk-83}
\end{figure}
...
\caption{RStudio Cloud, beta}\label{fig:unnamed-chunk-84}
\end{figure}
...
\caption{Initial View}\label{fig:unnamed-chunk-85}
\end{figure}
...
\caption{Packages Manager}\label{fig:unnamed-chunk-86}
\end{figure}
...
\caption{Install Dialog}\label{fig:unnamed-chunk-87}
\end{figure}
...
\caption{RStudio Desktop, Windows}\label{fig:unnamed-chunk-95}
\end{figure}
...
\caption{RStudio Pane Layout, Windows}\label{fig:unnamed-chunk-96}
\end{figure}
...
\caption{R Notebook file}\label{fig:unnamed-chunk-97}
\end{figure}
...
\caption{R Notebook saved file}\label{fig:unnamed-chunk-98}
\end{figure}
...
\caption{R Notebook insert chunk}\label{fig:unnamed-chunk-99}
\end{figure}
...
\caption{R Notebook first code}\label{fig:unnamed-chunk-100}
\end{figure}
...
\caption{R Notebook run code}\label{fig:unnamed-chunk-101}
\end{figure}
...
\caption{R Notebook option}\label{fig:unnamed-chunk-102}
\end{figure}
...
\caption{RStudio Cloud, beta}\label{fig:unnamed-chunk-103}
\end{figure}
...
\caption{Exploratory Public}\label{fig:unnamed-chunk-104}
\end{figure}
