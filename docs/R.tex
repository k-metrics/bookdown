...
インポートしたデータは必ずしもデータ分析に適した形式になっているとは限りませんので、\textbf{R}で扱いやすいような形式（Tidy Data）にします。\href{https://www.jstatsoft.org/article/view/v059i10}{Tidy Data}(Wickham 2014)はデータ分析において非成に重要な概念で、以下の条件を満たしたデータを意味します。
...
\item
  ...
  \tightlist
  \item
    様々なテキスト形式テーブルを読み込むためのパッケージ(Wickham and Hester 2020)
  \end{itemize}
\item
  \textbf{\texttt{readxl}}
  ...
  \tightlist
  \item
    Microsoft Excelのファイルを読み込むためのパッケージ(Wickham and Bryan 2019)
  \end{itemize}
\item
  \textbf{\texttt{rvest}}
  ...
  \tightlist
  \item
    Webスクレイピングのためのパッケージ(Wickham 2021b)
  \end{itemize}
\item
  ...
  \tightlist
  \item
    Google API v4 経由でスプレッドシートを読み込むためのパッケージ(Bryan 2021)
  \end{itemize}
\item
  \textbf{\texttt{DBI}}
  ...
  \tightlist
  \item
    SQL系の各種データベースと接続するためのパッケージ(R Special Interest Group on Databases (R-SIG-DB), Wickham, and Müller 2021)
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    Tidy Dataの作成を強力にサポートしてくれるパッケージ(Wickham 2021c)
  \end{itemize}
\item
  \textbf{\texttt{tibble}}
  ...
  \tightlist
  \item
    より厳密にTidy Dataを扱うためのパッケージ(Müller and Wickham 2021)
  \end{itemize}
\item
  \textbf{\texttt{zoo}}
  ...
  \tightlist
  \item
    時系列（TS）データを効率よく扱うためのパッケージ(Zeileis, Grothendieck, and Ryan 2021)
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    統一された文法で描画が行えるパッケージ(Wickham et al. 2020)
  \end{itemize}
\item
  ...
  \tightlist
  \item
    JavaScriptのウィジェットを利用できるパッケージ(Vaidyanathan et al. 2020)
  \end{itemize}
\item
  ...
  \tightlist
  \item
    \textbf{\texttt{ggplot2}}オブジェクトをひとつにまとめてレイアウトするためのパッケージ(Pedersen 2020)
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    Tidy Dataを様々な方法で操作するためのパッケージ(Wickham et al. 2021)
  \end{itemize}
\item
  \textbf{\texttt{stringr}}
...
\item
  ...
  \tightlist
  \item
    因子型を操作するためのパッケージ(Wickham 2021a)
  \end{itemize}
\item
  ...
  \tightlist
  \item
    日付データを簡単に変換するためのパッケージ(Spinu, Grolemund, and Wickham 2021)
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    各種モデリング結果を整然データにするためのパッケージ(Robinson, Hayes, and Couch 2021)
  \end{itemize}
\item
  ...
  \tightlist
  \item
    機械学習を中心としたモデリングフレームワーク(Kuhn and Wickham 2020, 2021)
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    \textbf{R}のコードと実行結果をPDFやHTMLなどに埋め込むためのパッケージ(Xie 2014, 2021b)
  \end{itemize}
\item
  ...
  \tightlist
  \item
    マークダウン書式を用いてレポートを作成するためのパッケージ(Allaire et al. 2021)
  \end{itemize}
\item
  ...
  \tightlist
  \item
    書籍や長いドキュメンを作成するためのパッケージ(Xie 2016, 2021a)
  \end{itemize}
\item
  \textbf{\texttt{shiny}}
  ...
  \tightlist
  \item
    インタラクティブなWebアプリケーションを作成するためのパッケージ(Chang et al. 2021)
  \end{itemize}
\end{itemize}
...
\item
  ...
  \tightlist
  \item
    パイプ演算子（\texttt{\%\textgreater{}\%}）によるスムースな処理を実現するパッケージ(Bache and Wickham 2020)
  \end{itemize}
\item
  \textbf{\texttt{purrr}}
  ...
  \tightlist
  \item
    関数型プログラミングによる反復処理を提供するパッケージ(Henry and Wickham 2020)
  \end{itemize}
\end{itemize}
...
\textbf{R}の文法説明が延々と続いてもつまらないので、本書では少し実用的な面から\textbf{R}のプログラミングを説明します。本チャプターでは『統計のはなし【改訂版】』(大村 2020)や『統計解析のはなし【改訂版】』(大平 2006)を参考書として説明します。個々の詳細については、これらの書籍などを参照してください。 なお、基本文法については\protect\hyperlink{Appendix-RBasics}{Appendix R Basics}をご覧ください。
...
\leavevmode\hypertarget{ref-RStartHere}{}%
