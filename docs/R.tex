...
\midrule
\endhead
ハイパーリンク\textsuperscript{a} & 脚注にURL表記 & CRAN\textsuperscript{No.} \\
パス・ファイル名 & モノフォント\textsuperscript{b} & \texttt{sample/sample.Rmd} \\
...
参考文献・資料\textsuperscript{c} & {[}著者・作者, 作成年{]} & {[}Wickham, 2021{]} \\
\bottomrule
\end{longtable}

\textsuperscript{a} PDF形式のみ、\textsuperscript{No.}の部分は章ごとの通し番号\\
\textsuperscript{b} タイプライタフォントとも呼ばれる等幅フォント\\
\textsuperscript{c} PDF形式のみ、{[}{]}内は文書内ハイパーリンク
...
環境によっては上記のように他のパッケージの関数をマスクしている旨のメッセージが出力されてますが、今は気にする必要ありません。 これで\textbf{\texttt{psych}}パッケージを使う準備が整いましたので、\texttt{geometric.mean()}関数で幾何平均を求めます。
