...
\tightlist
\item
  \textbf{\texttt{readr}} - 様々なテキスト形式テーブルを読み込むためのパッケージ\citep{R-readr}
\item
  \textbf{\texttt{readxl}} - Microsoft Excelのファイルを読み込むためのパッケージ\citep{R-readxl}
\item
  \textbf{\texttt{rvest}} - Webスクレイピングのためのパッケージ\citep{R-rvest}
\item
  \textbf{\texttt{googlesheets4}} - Google API v4 経由でスプレッドシートを読み込むためのパッケージ\citep{R-googlesheets4}
\item
  \textbf{\texttt{DBI}} - SQL系の各種データベースと接続するためのパッケージ\citep{R-DBI}
\end{itemize}
...
\item
  \textbf{\texttt{tibble}} - より厳密にTidy Dataを扱うためのパッケージ
\item
...
\item
  \textbf{\texttt{rmarkdown}} - マークダウン書式を用いてレポートを作成するためのパッケージ
\item
