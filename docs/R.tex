...
\hypertarget{rux306eux5fdcux7528}{%
\chapter{Rの応用}\label{rux306eux5fdcux7528}}

Data Science Workflowの個々のプロセスに入る前にもう少し\textbf{R}のプログラミングについて知って置きましょう。

\hypertarget{ux57faux672cux7d71ux8a08ux91cf}{%
\section{基本統計量}\label{ux57faux672cux7d71ux8a08ux91cf}}

データを分析する前には対象となるデータがどのような特徴を持っているか確認しておく必要があります。基本統計量と呼ばれる様々な統計量を用いるとデータの特徴が見えてきます。 ここでは『統計のはなし【改訂版】』（大村平著、日科技連出版社）

\hypertarget{ux5e73ux5747-1}{%
\subsection{平均}\label{ux5e73ux5747-1}}

統計量の中でよく使われるのが平均です。平均というと下式で表される算術平均を思い浮かべる方が多いと思いますが、 \[\bar{x} = \frac{x_1 + x_2 + \cdots + x_n}{n} = \frac{1}{n} \sum_{i = 1}^n x_i\] 算術平均以外にも下式で表される幾何平均（相乗平均）、 \[\sqrt[n]{x_1 \times x_2 \times \cdots \times x_n} = \sqrt[n]{\prod_{i = 1}^n x_i}\] 体操競技などで使われる複数審判が出す得点の最小値と最大値を除いた得点で求めるトリム平均などがあります。

\textbf{R}では算術平均とトリム平均は\texttt{mean()}関数で求めることができます。
...
算出平均は\texttt{mean()}関数に計算対象を指定します。
...
トリム平均は上記に加えて\texttt{trim}オプションで計算対象外にする割合を指定します。今回は五つのデータから最小値・最大値の二つを除いて求めたいので\texttt{0.25}という割合を指定します。

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean}\NormalTok{(x, }\AttributeTok{trim =} \FloatTok{0.25}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
幾何平均を求める関数は標準では用意されていません。「R geometric mean」で検索すればパッケージが見つかります。ここでは\textbf{\texttt{psych}}パッケージをインストールして使うことにします。パッケージをインストールして使えるようにするには以下の手順が必要です。
...
パッケージのインストールは\texttt{install.packages()}関数を用います。環境によってはCRANミラーサイトの選択を促される場合があります。その場合は、お住まいの地域に最も近いミラーサイトを選択してください。なお、パッケージのインストールは一度行えば済みます。
...
パッケージが提供する関数などを使えるようにするには\texttt{library()}関数を使って読み込みます。
...
この表記方法については賛否ありますが、どのパッケージの関数を使っているかを明示できるため、本書ではこの記述方法を用います。
...
レンジは比較的簡単に計算できますが、以下のように同じレンジを持つデータの特徴が同じとは言い難いです。
...
このような場合、下式で求められる標準偏差（Standard Deviation）を使います。 \[\sigma = \sqrt{\frac{1}{n} \sum_{i = 1}^n (x_i - \bar{x})^2} = \sqrt{\frac{\sum_{i = 1}^{n}x_{i}^2}{n} - \bar{x}^2 }\]
...
\caption{Google Colab, Theme: dark}\label{fig:unnamed-chunk-54}
\end{figure}
...
\caption{Upload notebook file}\label{fig:unnamed-chunk-55}
\end{figure}
...
\caption{Upload from GitHub}\label{fig:unnamed-chunk-56}
\end{figure}
...
\caption{Warning dialog}\label{fig:unnamed-chunk-57}
\end{figure}
...
\caption{RStudio Cloud, beta}\label{fig:unnamed-chunk-58}
\end{figure}
...
\caption{Initial View}\label{fig:unnamed-chunk-59}
\end{figure}
...
\caption{Packages Manager}\label{fig:unnamed-chunk-60}
\end{figure}
...
\caption{Install Dialog}\label{fig:unnamed-chunk-61}
\end{figure}
...
\caption{RStudio Desktop, Windows}\label{fig:unnamed-chunk-69}
\end{figure}
...
\caption{RStudio Pane Layout, Windows}\label{fig:unnamed-chunk-70}
\end{figure}
...
\caption{R Notebook file}\label{fig:unnamed-chunk-71}
\end{figure}
...
\caption{R Notebook saved file}\label{fig:unnamed-chunk-72}
\end{figure}
...
\caption{R Notebook insert chunk}\label{fig:unnamed-chunk-73}
\end{figure}
...
\caption{R Notebook first code}\label{fig:unnamed-chunk-74}
\end{figure}
...
\caption{R Notebook run code}\label{fig:unnamed-chunk-75}
\end{figure}
...
\caption{R Notebook option}\label{fig:unnamed-chunk-76}
\end{figure}
...
\caption{RStudio Cloud, beta}\label{fig:unnamed-chunk-77}
\end{figure}
...
\caption{Exploratory Public}\label{fig:unnamed-chunk-78}
\end{figure}
