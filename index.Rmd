...
lot: true                # 表一覧(List of tables)の出力
lof: true                # 図一覧(List of figures)の出力
# documentclass: krantz
fig_crop: false
fontsize: 12pt           # 本文のフォントサイズ
linestretch: 1.1         # 行間の指定
links-as-notes: true     # ハイパーリンク先を脚注に記載
# LaTeXのパッケージ指定とフォントは latex/preamble.tex に移動
# 以下はLaTeXでなくpandocのオプションらしい
...
knitr::opts_chunk$set(echo = TRUE,                 # Rコードを出力する
                      ...
                      out.width = "85%"            # 画像のデフォルトサイズ
                      )

# 出力がPDF(LaTeX)の場合は図をPDF形式としフォントをNotoにする、また、図の配置を
# 自動調整しないようにする
if (knitr::is_latex_output()) {
    knitr::opts_chunk$set(dev = "cairo_pdf",
                          ...
                          fig.pos = "H",
                          out.extra = "",
                          tinytex.verbose = TRUE   # コード折り返し設定
                          )
} else if (!knitr::is_html_output(excludes = "epub")) { #ePUB形式の場合
