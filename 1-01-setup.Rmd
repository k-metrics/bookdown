...
コンソールのキャレット（`>`表示）の部分に`2 * 3`と打ち込んで［Enter］キーを押して`[1] 6`と表示されることを確認してください。
...
以下のようなメッセージが表示されることを確認します。なお、インストール時期によってはバージョンなどが異なる場合があります。また、Windows 環境では一部の文字が化けます。
```{r, eval=FALSE}
