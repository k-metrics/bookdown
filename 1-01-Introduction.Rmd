...
* **`readr`** - 様々なテキスト形式テーブルを読み込むためのパッケージ[@R-readr]
* **`readxl`** - Microsoft Excelのファイルを読み込むためのパッケージ[@R-readxl]
* **`rvest`** - Webスクレイピングのためのパッケージ[@R-rvest]
* **`googlesheets4`** - Google API v4 経由でスプレッドシートを読み込むためのパッケージ[@R-googlesheets4]
* **`DBI`** - SQL系の各種データベースと接続するためのパッケージ[@R-DBI]


### Tidy {.unnumbered}

* **`tidyr`** - Tidy Dataの作成を強力にサポートしてくれるパッケージ
* **`tibble`** - より厳密にTidy Dataを扱うためのパッケージ
* **`zoo`** - 時系列（TS）データを効率よく扱うためのパッケージ


### Visualize {.unnumbered}
* **`ggplot2`** - 統一された文法で描画が行えるパッケージ
* **`htmlwidgets`** - JavaScriptのウィジェットを利用できるパッケージ


### Transform {.unnumbered}
* **`dplyr`** - Tidy Dataを様々な方法で操作するためのパッケージ
...
### Communicate {.unnumbered}
* **`knitr`** - **R**のコードと実行結果をPDFやHTMLなどに埋め込むためのパッケージ
* **`rmarkdown`** - マークダウン書式を用いてレポートを作成するためのパッケージ
* **`bookdown`** - 書籍や長いドキュメンを作成するためのパッケージ
* **`shiny`** - インタラクティブなWebアプリケーションを作成するためのパッケージ
