...
対象                 | 表記方法                            | 表記例
---------------------|-------------------------------------|-------------------
ハイパーリンク^a^    | 脚注にURL表記                       | CRAN^No.^
パス・ファイル名     | モノフォント^b^                     | `sample/sample.Rmd`
...
参考文献・資料^c^    | [著者・作者, 作成年]                | [Wickham, 2021]

^a^ PDF形式のみ、^No.^の部分は章ごとの通し番号  
^b^ タイプライタフォントとも呼ばれる等幅フォント  
^c^ PDF形式のみ、[]内は文書内ハイパーリンク  
...
### Tidy {.unnumbered}
インポートしたデータは必ずしもデータ分析に適した形式になっているとは限りませんので、**R**で扱いやすいような形式（Tidy Data）にします。[Tidy Data](https://www.jstatsoft.org/article/view/v059i10){target="_blank" title=""Jounal of Statistical Software}はデータ分析において非成に重要な概念で、以下の条件を満たしたデータを意味します。
