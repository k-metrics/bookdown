...
::: {.hint-box data-latex=""}
　以降に出てくる灰色に網掛けされて行番号がついている部分は**R**のコード、コードの下の`##`で始まる部分は実行結果（出力）です。コードによっては実行結果が出力されない場合もあります。
:::
...
```{r, echo=FALSE, eval=FALSE, fig.cap="花弁と萼片の分布"}
iris %>%
  tidyr::pivot_longer(cols = -Species, names_to = "part", values_to = "value") %>% 
...
```{r, echo=TRUE, eval=TRUE, fig.cap="花弁と萼片の分布"}
boxplot(iris[-5])
```

　数値で比較したように萼片（Sepal）より花弁（Petal）の方が小さいことがひと目で分かります。次に花弁（Petal）と萼片（Sepal）の幅と長さの関係を散布図で確認してみましょう。

```{r, echo=FALSE, eval=FALSE, fig.cap="幅と長さの関係", fig.subcap=c('花弁', '萼片'), fig.ncol=2, out.width="50%"}
iris %>%
...
```{r, fig.cap="花弁の幅と長さの関係"}
with(iris, plot(x = Petal.Width, y = Petal.Length))
```

```{r, fig.cap="萼片の幅と長さの関係"}
with(iris, plot(x = Sepal.Width, y = Sepal.Length))
```
...
　花弁（Petal）の方には強い相関（\index{そうかんけいすう@相関係数}相関係数$=`r with(iris, cor.test(Petal.Width, Petal.Length)$estimate)`$）があり、萼片（Sepal）の方には相関があるとは言えないことが分かります。  
...
```{r, echo=FALSE, eval=FALSE, message=FALSE, fig.cap="幅と長さの関係", fig.subcap=c('花弁の回帰モデル', '萼片の回帰モデル（参考）'), fig.ncol=2, out.width="50%"}
iris %>%
  ggplot2::ggplot(ggplot2::aes(x = Petal.Width, y = Petal.Length)) +
    ggplot2::geom_point() +
...
```{r}
car::scatterplot(Petal.Width ~ Petal.Length, data = iris, smooth = FALSE)
```
...
　Data Science Workflowを**R**で実現するための手段がHadley Wickhamが中心となって開発している**`tidyverse`** \index{Package tidyverse}パッケージ群による「Tidyverse Ecosystem」です。
...
* **Import**
  * **`readr`** [@R-readr] \index{Package readr}
      - 様々なテキスト形式テーブルを読み込むためのパッケージ
  * **`readxl`** [@R-readxl] \index{Package readxl}
      - Microsoft Excelのファイルを読み込むためのパッケージ
  * **`rvest`** [@R-rvest] \index{Package rvest}
      - Webスクレイピングのためのパッケージ
  * **`googlesheets4`** [@R-googlesheets4] \index{Package googlesheets4}
      - Google API v4 経由でスプレッドシートを読み込むためのパッケージ
  * **`DBI`** [@R-DBI] \index{Package DBI}
      - SQL系の各種データベースと接続するためのパッケージ

\newpage

* **Tidy**
  * **`tidyr`** [@R-tidyr] \index{Package tidyr}
      - Tidy Dataの作成を強力にサポートしてくれるパッケージ
  * **`tibble`** [@R-tibble] \index{Package tibble}
      - より厳密にTidy Dataを扱うためのパッケージ
  * **`zoo`** [@R-zoo] \index{Package zoo}
      - 時系列（TS）データを効率よく扱うためのパッケージ


* **Visualize**
  * **`ggplot2`** [@R-ggplot2] \index{Package ggplot2}
      - 統一された文法で描画が行えるパッケージ
  * **`htmlwidgets`** [@R-htmlwidgets] \index{Package htmlwidgets}
      - JavaScriptのウィジェットを利用できるパッケージ
  * **`patchwork`** [@R-patchwork] \index{Package patchwork}
      - **`ggplot2`**オブジェクトをひとつにまとめてレイアウトするためのパッケージ


* **Transform**
  * **`dplyr`** [@R-dplyr] \index{Package dplyr}
      - Tidy Dataを様々な方法で操作するためのパッケージ
  * **`stringr`** [@R-stringr] \index{Package stringr}
      - 文字列処理をするためのパッケージ
  * **`forcats`** [@R-forcats] \index{Package forcats}
      - 因子型を操作するためのパッケージ
  * **`lubridate`** [@R-lubridate] \index{Package lubridate}
      - 日付データを簡単に変換するためのパッケージ


* **Model**
  * **`broom`** [@R-broom] \index{Package broom}
      - 各種モデリング結果を整然データにするためのパッケージ
  * **`tidymodels`**[@tidymodels2020; @R-tidymodels] \index{Package tidymodels}
      - 機械学習を中心としたモデリングフレームワーク

\newpage

* **Communicate**
  * **`knitr`** [@knitr2014; @R-knitr] \index{Package knitr}
      - **R**のコードと実行結果をPDFやHTMLなどに埋め込むためのパッケージ
  * **`rmarkdown`** [@R-rmarkdown] \index{Package rmarkdown}
      - マークダウン書式を用いてレポートを作成するためのパッケージ
  * **`bookdown`** [@bookdown2016; @R-bookdown] \index{Package bookdown}
      - 書籍や長いドキュメンを作成するためのパッケージ
  * **`shiny`** [@R-shiny] \index{Package shiny}
      - インタラクティブなWebアプリケーションを作成するためのパッケージ

　プロセスのハブ・スポークを担うパッケージもEcosystemの一部です。

* **Othres**
  * **`magrittr`** [@R-magrittr] \index{Package magrittr}
      - パイプ演算子（`%>%`）によるスムースな処理を実現するパッケージ
  * **`purrr`** [@R-purrr] \index{Package purrr}
