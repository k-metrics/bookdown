...
データ分析を行うためは分析ツールが必要不可欠で、その使い方を知っているのと知らないのでは分析効率や**再現性確保に差**が生じます。特にソフトウェアメトリクス分析のようなアドホック分析ではその差が顕著になります。
...
## なぜRを使うのか
メトリクス分析のようなアドホック分析を行うのに適したツール（言語）に R言語（以降、R） があります。Rを使う理由を簡潔に説明しているのが ["Six Reasons To Learn R For Business", R Blogger](https://www.r-bloggers.com/six-reasons-to-learn-r-for-business/){target="_blank"} です。
...
## RStudioを使おう
[データ分析勉強会]() では、ながらく [R Commander（以降、Rcmdr）]() が利用されてきました。Rcmdrはコードを記載しなくても可視化や分析ができますので初学者にとっては便利な道具です。しかし、RcmdrはRのごく一部のコマンド（関数）をGUIで使えるようにしたラッパーですので、できることが非常に限られています。加えてGUI操作が記録に残りませんので分析再現性の観点からみても好ましいとは言えません。
